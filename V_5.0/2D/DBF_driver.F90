...
contains
    ...
    subroutine initialize()
        ...
        if(myid == 0) then
            ...
            open(unit=48, file=trim(adjustl(soludoc))//'/his_conError.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_conError.txt', ' error. ', ierr
                stop
            end if
            open(unit=49, file=trim(adjustl(soludoc))//'/his_inAcid.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_inAcid.txt', ' error. ', ierr
                stop
            end if
            open(unit=50, file=trim(adjustl(soludoc))//'/his_conAcid.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_conAcid.txt', ' error. ', ierr
                stop
            end if
            open(unit=51, file=trim(adjustl(soludoc))//'/his_remainAcid.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_remainAcid.txt', ' error. ', ierr
                stop
            end if
            open(unit=52, file=trim(adjustl(soludoc))//'/his_outAcid.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_outAcid.txt', ' error. ', ierr
                stop
            end if
            open(unit=53, file=trim(adjustl(soludoc))//'/his_upSw.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_upSw.txt', ' error. ', ierr
                stop
            end if
            open(unit=54, file=trim(adjustl(soludoc))//'/his_middleSw.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_middleSw.txt', ' error. ', ierr
                stop
            end if
            open(unit=55, file=trim(adjustl(soludoc))//'/his_downSw.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_downSw.txt', ' error. ', ierr
                stop
            end if
            open(unit=56, file=trim(adjustl(soludoc))//'/his_presDrop.txt', status='replace', iostat=ierr)
            if(ierr /= 0) then
                print *, 'open file ', trim(adjustl(soludoc))//'/his_presDrop.txt', ' error. ', ierr
                stop
            end if
        end if
    ...
    subroutine computePresDrop(isBT)
        ...
        if(myid == 0) then
            ...
            write(56, fmt='(es24.16)', iostat=ierr) presDrop/presDropInit
            if(ierr /= 0) then
                print *, 'write file error. ', ierr
                stop
            end if
    ...
    subroutine computeConOutAcid()
        ...
        do j = 1, localnrows
            do i = 1, localncols
                ...
                localConAcid = localConAcid + av(i,j)*kc(i,j)*ks(i,j)/(kc(i,j)+ks(i,j))*Cf(i,j)
        ...
        if(pcol == pncols) then
            do j = 1, localnrows
                localOutAcid = localOutAcid + max(vxw(localncols+1,j),0.D0)*(timeEnd/nt)*(Ly/ny)*Cf(localncols,j)
            end do
        end if

    end subroutine computeConOutAcid

    subroutine checkConservation()

        real(kind=8), dimension(:), allocatable :: local_data, recv
        real(kind=8) :: totalConAcid, totalRemainAcid, localRemainAcid, totalInAcid, totalOutAcid, conError
        integer :: status(MPI_STATUS_SIZE)
        integer :: request, ierr, i, j

        localRemainAcid = 0
        do j = 1, localnrows
            do i = 1, localncols

                localRemainAcid = localRemainAcid + poro(i,j)*Sw(i,j)*Cf(i,j)

            end do
        end do
        localRemainAcid = localRemainAcid*(Lx/nx)*(Ly/ny)

        allocate(local_data(3))
        local_data(1) = localConAcid
        local_data(2) = localRemainAcid
        local_data(3) = localOutAcid

        if(myid /= 0) then
            call MPI_IBSEND(local_data, 3, MPI_DOUBLE_PRECISION, 0, myid, MPI_COMM_WORLD, request, ierr)
        end if

        if(myid == 0) then

            totalConAcid = localConAcid
            totalRemainAcid = localRemainAcid
            totalOutAcid = localOutAcid
            allocate(recv(3))
            do i = 1, nProcs-1
                call MPI_RECV(recv, 3, MPI_DOUBLE_PRECISION, i, i, MPI_COMM_WORLD, status, ierr)
                totalConAcid = totalConAcid + recv(1)
                totalRemainAcid = totalRemainAcid + recv(2)
                totalOutAcid = totalOutAcid + recv(3)
            end do
            deallocate(recv)

            totalConAcid = totalConAcid*(Lx/nx)*(Ly/ny)*(timeEnd/nt)

            totalInAcid = CfBdryX0(1)*vxwBdryX0(1)*Ly*(timeEnd/nt)*(t-1)

            conError = abs(totalInAcid - totalConAcid - totalRemainAcid - totalOutAcid) / totalInAcid
            if(mod(t, 1000) == 0) then
                print *, 'The conservation error = ', conError*1.D2, '%', totalInAcid, totalConAcid, totalRemainAcid, totalOutAcid
            end if

            write(48, fmt='(es24.16)', iostat=ierr) conError
            if(ierr /= 0) then
                print *, 'write file error. ', ierr
                stop
            end if
            write(49, fmt='(es24.16)', iostat=ierr) totalInAcid
            if(ierr /= 0) then
                print *, 'write file error. ', ierr
                stop
            end if
            write(50, fmt='(es24.16)', iostat=ierr) totalConAcid
            if(ierr /= 0) then
                print *, 'write file error. ', ierr
                stop
            end if
            write(51, fmt='(es24.16)', iostat=ierr) totalRemainAcid
            if(ierr /= 0) then
                print *, 'write file error. ', ierr
                stop
            end if
            write(52, fmt='(es24.16)', iostat=ierr) totalOutAcid
            if(ierr /= 0) then
                print *, 'write file error. ', ierr
                stop
            end if
    ...
    subroutine codeVerification()

        real(kind=8) :: totalColSw, aveColSw
        integer :: ierr, j

        if(nProcs /= 1) then
            print *, 'The codeVerification subroutine should run when there is only one process!'
            stop
        end if

        totalColSw = 0.D0
        do j = 1, ny
            totalColSw = totalColSw + Sw(1,j)
        end do
        aveColSw = totalColSw / ny

        ! up point
        write(53, fmt='(es24.16)', iostat=ierr) abs(Sw(1,3*ny/4)-aveColSw)/aveColSw
        if(ierr /= 0) then
            print *, 'write file error. ', ierr
            stop
        end if
        ...
        ! middle point
        write(54, fmt='(es24.16)', iostat=ierr) abs(Sw(1,ny/2)-aveColSw)/aveColSw
        if(ierr /= 0) then
            print *, 'write file error. ', ierr
            stop
        end if
        ...
        ! down point
        write(55, fmt='(es24.16)', iostat=ierr) abs(Sw(1,ny/4)-aveColSw)/aveColSw
        if(ierr /= 0) then
            print *, 'write file error. ', ierr
            stop
        end if

    end subroutine codeVerification
...
!            call genDynPara_Cf()
 ...
 !           call computeTem()
            ...
            call computeConOutAcid()
            call checkConservation()
            call codeVerification()
