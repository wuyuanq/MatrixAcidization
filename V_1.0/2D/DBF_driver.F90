...
contains
    ...
    subroutine initialize(modelCase)
        ...
        integer :: xmomeSize, ymomeSize, contiSize, cfSize
        ...
        call HYPRE_ParCSRFlexGMRESCreate(MPI_COMM_WORLD, solver, ierr)
        call HYPRE_ParCSRFlexGMRESSetKDim(solver, 50, ierr)
        call HYPRE_ParCSRFlexGMRESSetMaxIter(solver, 1000, ierr)
        call HYPRE_ParCSRFlexGMRESSetTol(solver, 1.0e-7, ierr)
        call HYPRE_ParCSRFlexGMRESSetPrintLevel(solver, 3, ierr)
        call HYPRE_ParCSRFlexGMRESSetPrecond(solver, 0, precond, ierr)
    ...
    subroutine computekc()
        ...
        do j = 1, localnrows
            do i = 1, localncols
                ...
                vmodulus = dsqrt(((vx(i+1,j)-vx(i,j))**2.D0+(vy(i,j+1)-vy(i,j))**2.D0)*1.D0)
                ...
                kc(i,j) = (ShInfinity+7.D-1*Rep**(1.D0/2.D0)*Sc**(1.D0/3.D0))*dm/2.D0/radius
    ...
    subroutine computePoro()
        ...
        do j = 1, localnrows
            do i = 1, localncols
                coe = avInit(xlower+i-1,ylower+j-1)*al*Cf(i,j)*kc(i,j)*ks*(ts(t)-ts(t-1))/ &!
                    (rhos*(kc(i,j)+ks)*(1.D0-poroInit(xlower+i-1,ylower+j-1)))
                poro_old(i,j) = poro(i,j)
        ...
        do j = 1, localnrows
            do i = 1, localncols
                ...
                local_rhs(c) = local_rhs_static(c) - rhs_dpdt(i,j) - &!
                    kc(i,j)*ks*Cf(i,j)*av(i,j)*al/rhos/(kc(i,j)+ks)
            end do
        end do
        deallocate(rhs_dpdt)
    ...
    subroutine outputHisData()
        ...
        integer :: status(MPI_STATUS_SIZE)
        integer :: request
        real(kind=8) :: poroavg, Kxxavg, avavg, qavg, pavg, Cfavg
    ...
    subroutine outputRawData()
        ...
        local_data_size = local_x_size+3*localncols*localnrows
        allocate(local_data(local_data_size))
        ...
        if(myid == 0) then
            ...
            allocate(global_data((nx+1)*ny+nx*(ny+1)+4*nx*ny))

            global_data(1:local_data_size) = local_data(1:local_data_size)
            c = local_data_size + 1
            do i = 1, nProcs-1
                slave_data_size = slave_vp_data_size(i)+3*localncols*localnrows
    ...
    subroutine finalize()
        ...
        call HYPRE_ParCSRFlexGMRESDestroy(solver, ierr)
