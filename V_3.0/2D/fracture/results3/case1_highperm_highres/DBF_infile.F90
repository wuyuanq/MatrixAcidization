...
program DBF_infile
    ...
    deallocate(random)
    ! add the fracture
    do i = 10*times+1, 20*times
        j = 30*times - 2*(i-(10*times+1))
        modelCase%poroInit(i,j) = 0.7
        modelCase%poroInit(i,j-1) = 0.7
    end do
    do i = 30*times+1, 40*times
        j = 10*times+1 + 2*(i-(30*times+1))
        modelCase%poroInit(i,j) = 0.7
        modelCase%poroInit(i,j+1) = 0.7
    end do
    do i = 60*times+1, 70*times
        j = 30*times - 2*(i-(60*times+1))
        modelCase%poroInit(i,j) = 0.7
        modelCase%poroInit(i,j-1) = 0.7
    end do
    do i = 80*times+1, 90*times
        j = 10*times+1 + 2*(i-(80*times+1))
        modelCase%poroInit(i,j) = 0.7
        modelCase%poroInit(i,j+1) = 0.7
    end do

    allocate(modelCase%KxxInit(modelCase%nx,modelCase%ny))
    modelCase%KxxInit(:,:) = 1.D-14!9.869233D-16
    allocate(modelCase%KyyInit(modelCase%nx,modelCase%ny))
    modelCase%KyyInit(:,:) = 1.D-14!9.869233D-16
    ! add the fracture
    do i = 10*times+1, 20*times
        j = 30*times - 2*(i-(10*times+1))
        modelCase%KxxInit(i,j) = 1.D-11
        modelCase%KxxInit(i,j-1) = 1.D-11
        modelCase%KyyInit(i,j) = 1.D-11
        modelCase%KyyInit(i,j-1) = 1.D-11
    end do
    do i = 30*times+1, 40*times
        j = 10*times+1 + 2*(i-(30*times+1))
        modelCase%KxxInit(i,j) = 1.D-11
