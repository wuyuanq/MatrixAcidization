...
contains
    ...
    subroutine exportResults(global_data)
        ...
        character(len=40) :: fporotxt, fKxxtxt, fvxtxt, fvytxt, fvztxt, fptxt, &!
            fCftxt, fTemtxt, fvxmidtxt, fvymidtxt
        integer :: pid, ierr
        integer :: i, j, k, c
        ...
        do pid = 0, nProcs-1
            ...
            do k = 1, localnlays
                do j = 1, localnrows
                    do i = 1, localncols
                        c = c + 1
                        g_Tem(p_xlower+i-1,p_ylower+j-1,p_zlower+k-1) = global_data(c)
                    end do
                end do
            end do
        ...
        do k = 1, nz
            do j = 1, ny
                do i = 1, nx
                    write(10, fmt='(es24.16e3)', iostat=ierr) g_Tem(i,j,k)
                    if(ierr /= 0) then
        ...
        call export2tecplot(g_poro, g_Kxx, g_vx, g_vy, g_vz, g_p, g_Cf, g_Tem)
