...
contains
    ...
    subroutine initialize()
        ...
        integer :: xmomSize, ymomSize, conSize, CfSize, SwSize
    ...
    subroutine computekc()
        ...
        do j = 1, localnrows
            do i = 1, localncols
                ...
                vwmodulus = dsqrt(((vxw(i+1,j)-vxw(i,j))**2.D0+(vyw(i,j+1)-vyw(i,j))**2.D0)*1.D0)
    ...
    subroutine computePoro()
        ...
        do j = 1, localnrows
            do i = 1, localncols
                poro_old(i,j) = poro(i,j)
                poro(i,j) = poro_old(i,j) + Sw(i,j)*av(i,j)*al*Cf(i,j)*kc(i,j)*ks*(ts(t)-ts(t-1))/(rhos*(kc(i,j)+ks))
            end do
        end do
    ...
    subroutine outputRawData()
        ...
        local_data_size = local_x_size_vp+4*localncols*localnrows
        allocate(local_data(local_data_size))
        ...
        if(myid == 0) then
            ...
            allocate(global_data((nx+1)*ny*2+nx*(ny+1)*2+5*nx*ny))
