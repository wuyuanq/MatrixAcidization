...
contains
    ...
    subroutine computeMatEntryNum()
        ...
        if(pcol == 1) then
            do k = 1, localnlays
                do j = 1, localnrows
                    call equCoorditoInd(5, 1, j, k, eq_ind)
        ...
        if(pcol == pncols) then
            do k = 1, localnlays
                do j = 1, localnrows
                    call equCoorditoInd(5, localncols, j, k, eq_ind)
                    AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) - 5
                end do
            end do
        end if
        if(prow == 1) then
            do k = 1, localnlays
                do i = 1, localncols
                    call equCoorditoInd(5, i, 1, k, eq_ind)
                    AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) - 5
                end do
            end do
        end if
        if(prow == pnrows) then
            do k = 1, localnlays
                do i = 1, localncols
                    call equCoorditoInd(5, i, localnrows, k, eq_ind)
                    AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) - 5
                end do
            end do
        end if
        if(play == 1) then
            do j = 1, localnrows
                do i = 1, localncols
                    call equCoorditoInd(5, i, j, 1, eq_ind)
                    AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) - 5
                end do
            end do
        end if
        if(play == pnlays) then
            do j = 1, localnrows
                do i = 1, localncols
                    call equCoorditoInd(5, i, j, localnlays, eq_ind)
                    AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) - 5
                end do
            end do
        end if
        if((pcol==1).and.(prow==1)) then
            do k = 1, localnlays
                call equCoorditoInd(5, 1, 1, k, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==1).and.(prow==pnrows)) then
            do k = 1, localnlays
                call equCoorditoInd(5, 1, localnrows, k, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==pncols).and.(prow==1)) then
            do k = 1, localnlays
                call equCoorditoInd(5, localncols, 1, k, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==pncols).and.(prow==pnrows)) then
            do k = 1, localnlays
                call equCoorditoInd(5, localncols, localnrows, k, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==1).and.(play==1)) then
            do j = 1, localnrows
                call equCoorditoInd(5, 1, j, 1, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==1).and.(play==pnlays)) then
            do j = 1, localnrows
                call equCoorditoInd(5, 1, j, localnlays, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==pncols).and.(play==1)) then
            do j = 1, localnrows
                call equCoorditoInd(5, localncols, j, 1, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((pcol==pncols).and.(play==pnlays)) then
            do j = 1, localnrows
                call equCoorditoInd(5, localncols, j, localnlays, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((prow==1).and.(play==1)) then
            do i = 1, localncols
                call equCoorditoInd(5, i, 1, 1, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((prow==1).and.(play==pnlays)) then
            do i = 1, localncols
                call equCoorditoInd(5, i, 1, localnlays, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((prow==pnrows).and.(play==1)) then
            do i = 1, localncols
                call equCoorditoInd(5, i, localnrows, 1, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        if((prow==pnrows).and.(play==pnlays)) then
            do i = 1, localncols
                call equCoorditoInd(5, i, localnrows, localnlays, eq_ind)
                AcfEntryNum(eq_ind) = AcfEntryNum(eq_ind) + 1
            end do
        end if
        ...
        if(pcol == 1) then
            do k = 1, localnlays
                do j = 1, localnrows
                    call equCoorditoInd(6, 1, j, k, eq_ind)
        ...
        if(pcol == pncols) then
            do k = 1, localnlays
                do j = 1, localnrows
                    call equCoorditoInd(6, localncols, j, k, eq_ind)
                    AtemEntryNum(eq_ind) = AtemEntryNum(eq_ind) - 1
                end do
            end do
        end if
        ...
        if(prow == 1) then
            do k = 1, localnlays
                do i = 1, localncols
                    call equCoorditoInd(6, i, 1, k, eq_ind)
                    AtemEntryNum(eq_ind) = AtemEntryNum(eq_ind) - 1
                end do
            end do
        end if
        ...
        if(prow == pnrows) then
            do k = 1, localnlays
                do i = 1, localncols
                    call equCoorditoInd(6, i, localnrows, k, eq_ind)
                    AtemEntryNum(eq_ind) = AtemEntryNum(eq_ind) - 1
                end do
            end do
        end if
        ...
        if(play == 1) then
            do j = 1, localnrows
                do i = 1, localncols
                    call equCoorditoInd(6, i, j, 1, eq_ind)
                    AtemEntryNum(eq_ind) = AtemEntryNum(eq_ind) - 1
                end do
            end do
        end if
        ...
        if(play == pnlays) then
            do j = 1, localnrows
                do i = 1, localncols
                    call equCoorditoInd(6, i, j, localnlays, eq_ind)
                    AtemEntryNum(eq_ind) = AtemEntryNum(eq_ind) - 1
                end do
            end do
        end if
    ...
    subroutine initialize(modelCase)
        ...
        integer :: xmomeSize, ymomeSize, zmomeSize, contiSize, cfSize, temSize
        ...
        call index_convert_local_global(myid, 6, 1, 1, 1, ilower_Tem)
        call index_convert_local_global(myid, 6, localncols, localnrows, localnlays, iupper_Tem)
        ...
        ! initialize Tem
        allocate(Tem(1:localncols,1:localnrows,1:localnlays))
        Tem(1:localncols,1:localnrows,1:localnlays) = TemInit(xlower:xupper,ylower:yupper,zlower:zupper)
        ...
        temSize = localncols*localnrows*localnlays
        ...
        allocate(Ai_Tem(1:7*local_x_size_Tem))
        allocate(Ax_Tem(1:7*local_x_size_Tem))
        ...
        if(mumps_par_Tem%MYID == 0) then
            mumps_par_Tem%N = nx*ny*nz
        end if
        allocate(mumps_par_Tem%RHS(mumps_par_Tem%N))

        allocate(mumps_IRN_loc_Tem(local_x_size_Tem*7))
        allocate(mumps_JCN_loc_Tem(local_x_size_Tem*7))
        allocate(mumps_A_loc_Tem(local_x_size_Tem*7))
        ...
        if(play /= 1) then
            call index_convert_local_global(myid-pncols*pnrows, 6, 1, 1, localnlays, jlower_Tem)
        elseif(prow /= 1) then
            call index_convert_local_global(myid-pncols, 6, 1, localnrows, 1, jlower_Tem)
        elseif(pcol /= 1) then
            call index_convert_local_global(myid-1, 6, localncols, 1, 1, jlower_Tem)
        else
            jlower_Tem = ilower_Tem
        end if
        if(play /= pnlays) then
            call index_convert_local_global(myid+pncols*pnrows, 6, localncols, localnrows, 1, jupper_Tem)
        elseif(prow /= pnrows) then
            call index_convert_local_global(myid+pncols, 6, localncols, 1, localnlays, jupper_Tem)
        elseif(pcol /= pncols) then
            call index_convert_local_global(myid+1, 6, 1, localnrows, localnlays, jupper_Tem)
        else
        ...
        do k = 1, localnlays
            do j = 1, localnrows
                do i = 1, localncols
                    c = c + 1
                    initial_x_guess_Tem(c) = TemInit(xlower+i-1,ylower+j-1,zlower+k-1)
                end do
            end do
        end do
            ...
            do j = 1, localnrows
                do i = 1, localncols
                    coe = avInit(xlower+i-1,ylower+j-1,zlower+k-1)*al*Cf(i,j,k)*kc(i,j,k)*ks(i,j,k)*(ts(t)-ts(t-1))/ &!
                        (rhos*(kc(i,j,k)+ks(i,j,k))*(1-poroInit(xlower+i-1,ylower+j-1,zlower+k-1)))
                    poro_old(i,j,k) = poro(i,j,k)
            ...
            do j = 1, localnrows
                do i = 1, localncols
                    ...
                    local_rhs(c) = local_rhs_static(c) - rhs_dpdt(i,j,k) - &!
                        kc(i,j,k)*ks(i,j,k)*Cf(i,j,k)*av(i,j,k)*al/rhos/(kc(i,j,k)+ks(i,j,k))
                end do
            end do
        end do
        deallocate(rhs_dpdt)
    ...
    subroutine genDynPara_Tem()
        ...
        findexf = 0
        findexb = localnlays + 1
        allocate(tempe(findexl:findexr,findexd:findexu,findexf:findexb))
        tempe(:,:,:) = 0
        ...
        eindexf = 1
        eindexb = localnlays
        allocate(rhs_Tem(eindexl:eindexr,eindexd:eindexu,eindexf:eindexb))
        rhs_Tem(:,:,:) = 0.D0

        call Resi_Tem(tempe, rhs_Tem)

        allocate(resiAtem(eindexl:eindexr,eindexd:eindexu,eindexf:eindexb))
        allocate(resitemp(eindexl:eindexr,eindexd:eindexu,eindexf:eindexb))
        do k = findexl, findexr
            do j = findexd, findexd+2
                do i = findexl, findexl+2
                    call genExpField(i, j, k, findexr, findexu, findexb, tempe, isField)
                    if(isField) then
    ...
    subroutine computeTem()
        ...
        if(mumps_par_Tem%MYID == 0) then
            ...
            do i = 1, nProcs-1
                ...
                call MPI_IBSEND(slave_data, local_x_size_Tem, MPI_DOUBLE_PRECISION, &!
                    i, myid, MPI_COMM_WORLD, requestarray(i), ierr)
                c = c + local_x_size_Tem
                deallocate(slave_data)
            end do
        end if
        ...
        if(mumps_par_Tem%MYID /= 0) then
            allocate(slave_data(local_x_size_Tem))
            call MPI_RECV(slave_data, local_x_size_Tem, MPI_DOUBLE_PRECISION, &!
    ...
    subroutine outputRawData()
        ...
        local_data_size = local_x_size+4*localncols*localnrows*localnlays
        allocate(local_data(local_data_size))
        ...
        if(myid == 0) then
            ...
            c = local_data_size + 1
            do i = 1, nProcs-1
                slave_data_size = slave_vp_data_size(i)+4*localncols*localnrows*localnlays
