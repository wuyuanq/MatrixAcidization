...
Contains
            ...
            do i = indexl, indexr
                ...
                if(conc(i,j) == 1) then
                    ...
                    elseif((i==1).and.(j==1)) then
                        call index_convert_local_global(myid, 4, 1, 1, fieldInd)
                        call index_convert_local_global(myid, 4, 1, 1, equInd)
                        call setMatValue(fieldInd, equInd, resi(1,1), 10, 1, 1)
                        call index_convert_local_global(myid, 4, 2, 1, equInd)
                        call setMatValue(fieldInd, equInd, resi(2,1), 10, 2, 1)
                        call index_convert_local_global(myid, 4, 1, 2, equInd)
                        call setMatValue(fieldInd, equInd, resi(1,2), 10, 1, 2)
                        call index_convert_local_global(myid, 4, 2, 2, equInd)
                        call setMatValue(fieldInd, equInd, resi(2,2), 10, 2, 2)
                    elseif((i==1).and.(j==localnrows)) then
                        call index_convert_local_global(myid, 4, 1, localnrows, fieldInd)
                        call index_convert_local_global(myid, 4, 1, localnrows-1, equInd)
                        call setMatValue(fieldInd, equInd, resi(1,localnrows-1), 10, 1, localnrows-1)
                        call index_convert_local_global(myid, 4, 2, localnrows-1, equInd)
                        call setMatValue(fieldInd, equInd, resi(2,localnrows-1), 10, 2, localnrows-1)
                        call index_convert_local_global(myid, 4, 1, localnrows, equInd)
                        call setMatValue(fieldInd, equInd, resi(1,localnrows), 10, 1, localnrows)
                        call index_convert_local_global(myid, 4, 2, localnrows, equInd)
                        call setMatValue(fieldInd, equInd, resi(2,localnrows), 10, 2, localnrows)
                    elseif((i==localncols).and.(j==1)) then
                        call index_convert_local_global(myid, 4, localncols, 1, fieldInd)
                        call index_convert_local_global(myid, 4, localncols-1, 1, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols-1,1), 10, localncols-1, 1)
                        call index_convert_local_global(myid, 4, localncols, 1, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols,1), 10, localncols, 1)
                        call index_convert_local_global(myid, 4, localncols-1, 2, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols-1,2), 10, localncols-1, 2)
                        call index_convert_local_global(myid, 4, localncols, 2, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols, 2), 10, localncols, 2)
                    elseif((i==localncols).and.(j==localnrows)) then
                        call index_convert_local_global(myid, 4, localncols, localnrows, fieldInd)
                        call index_convert_local_global(myid, 4, localncols-1, localnrows-1, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols-1,localnrows-1), 10, localncols-1, localnrows-1)
                        call index_convert_local_global(myid, 4, localncols, localnrows-1, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols,localnrows-1), 10, localncols, localnrows-1)
                        call index_convert_local_global(myid, 4, localncols-1, localnrows, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols-1,localnrows), 10, localncols-1, localnrows)
                        call index_convert_local_global(myid, 4, localncols, localnrows, equInd)
                        call setMatValue(fieldInd, equInd, resi(localncols, localnrows), 10, localncols, localnrows)
            ...
            do i = indexl, indexr
                ...
                if(tempe(i,j) == 1) then
                    ...
                    if((i==0).and.(j/=0).and.(j/=localnrows+1)) then
                        ...
                        call index_convert_local_global(myid, 5, 1, j, equInd)
                        call setMatValue(fieldInd, equInd, resi(i+1,j), 11, 1, j)
                    elseif((i==localncols+1).and.(j/=0).and.(j/=localnrows+1)) then
                        call index_convert_local_global(myid+1, 5, 1, j, fieldInd)
                        call index_convert_local_global(myid, 5, localncols, j, equInd)
                        call setMatValue(fieldInd, equInd, resi(i-1,j), 11, localncols, j)
                    elseif((j==0).and.(i/=0).and.(i/=localncols+1)) then
                        call index_convert_local_global(myid-pncols, 5, i, localnrows, fieldInd)
                        call index_convert_local_global(myid, 5, i, 1, equInd)
                        call setMatValue(fieldInd, equInd, resi(i,j+1), 11, i, 1)
                    elseif((j==localnrows+1).and.(i/=0).and.(i/=localncols+1)) then
                        call index_convert_local_global(myid+pncols, 5, i, 1, fieldInd)
                        call index_convert_local_global(myid, 5, i, localnrows, equInd)
                        call setMatValue(fieldInd, equInd, resi(i,j-1), 11, i, localnrows)
                    elseif((i>=1).and.(i<=localncols).and.(j>=1).and.(j<=localnrows)) then
                        call index_convert_local_global(myid, 5, i, j, fieldInd)
                        call setMatValue(fieldInd, fieldInd, resi(i,j), 11, i, j)
                        if(i /= 1) then
                            call index_convert_local_global(myid, 5, i-1, j, equInd)
                            call setMatValue(fieldInd, equInd, resi(i-1,j), 11, i-1, j)
                        end if
                        ...
                        if(j /= 1) then
                            ...
                            call setMatValue(fieldInd, equInd, resi(i,j-1), 11, i, j-1)
                        end if
    ...
    subroutine index_convert_local_global(pid, kind, local_i, local_j, global_ind)
        ...
        integer :: base, ubase, vbase, Cfbase, Tembase
        ...
        Tembase = (p_prow-1)*nx*localnrows + (p_pcol-1)*localncols*localnrows
        ...
        ! the 'Tem' index
        elseif(kind == 5) then
            global_ind = Tembase + (local_j-1)*localncols + local_i
        end if
